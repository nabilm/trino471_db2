        ...
        implements Module
{
    ...
    @Provides
    @Singleton
    public static CredentialPropertiesProvider getCredentialPropertiesProvider(CredentialProvider credentialProvider)
    {
        return new DefaultCredentialPropertiesProvider(credentialProvider);
    }

    @Provides
    @Singleton
    @ForBaseJdbc
    public static ConnectionFactory getConnectionFactory(BaseJdbcConfig config, CredentialPropertiesProvider credentialProvider, DB2Config db2Config, OpenTelemetry openTelemetry)
    {
        ...
        return new DriverConnectionFactory(new DB2Driver(), config.getConnectionUrl(), connectionProperties, credentialProvider, openTelemetry);
    }
}
